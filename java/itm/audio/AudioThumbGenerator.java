...
public class AudioThumbGenerator {
	...
	protected File processAudio(File input, File output) throws IOException,
		...
		AudioInputStream shortAudioInputStream = null;
		...
		try{
			...
			//Audioformat format = new Audioformat (encoding, sampleRate, SampleSizein Bits, channels *2 because stereo framesize,
			// frameRate, bigEndian)
			...
			// get the AudioInputstream  for decodedFormat and input
			din = AudioSystem.getAudioInputStream(decodedFormat,audioInputStream);


			// cut the audio data in the stream to a given length
			long interval = (int)decodedFormat.getFrameRate() * this.thumbNailLength;

			shortAudioInputStream = new AudioInputStream(din,decodedFormat,interval);


			// save the acoustic thumbnail as WAV file
			//putting the audiostream in shortenedStream with correct format
			AudioSystem.write(shortAudioInputStream,AudioFileFormat.Type.WAVE, outputFile);

		}catch (Exception e){
			System.out.println(e);
		} finally {
			if(shortAudioInputStream != null) try {
				shortAudioInputStream.close();
			}catch (Exception e){
				System.out.println(e);
			}
		}
