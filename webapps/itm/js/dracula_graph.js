...
EdgeFactory.prototype = {
    build: function(source, target, url) {
...
Graph.prototype = {
     ...
     *              representation
     */
    addNode: function(id, content, url) {
        /* testing if node is already existing in the graph */
        if(this.nodes[id] == undefined) {
            this.nodes[id] = new Graph.Node(id, content, url);
        }
        return this.nodes[id];
    },

    addEdge: function(source, target, url, style) {
        var s = this.addNode(source, "", url);
        var t = this.addNode(target, "", url);
        var edge = this.edgeFactory.build(s, t, url);
 ...
 * Node
 */
Graph.Node = function(id, node, url){
    node = node || {};
    node.id = id;
    node.url = url;
    node.edges = [];
    ...
    drawNode: function(node, url) {
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                var ellipse = r.ellipse(0, 0, 30, 20).attr({fill: color, stroke: color, "stroke-width": 2, "href": node.url});
                /* set DOM node ID */
